pipeline {
    ...
    environment {
        VERSION = "latest"
        DOCKERHUB_REPOSITORY_BACK = "ynwoo/tunemate-gateway"
        DOCKERHUB_CREDENTIAL = credentials('dockerhub-ynwoo')
        CONTAINER_NAME_BACK = "gateway"
        SSH_CONNECTION = "ubuntu@k9a603.p.ssafy.io"
        ENV_DIR = "./config/.env"
        PORT_BACK = "8000"
    }
    stages {
        stage('Build Gateway Backend') {
            steps {
                dir('tunemate-api-gateway') {
                    sh "chmod +x gradlew"
        ...
        stage('Deploy Backend Server') {
            steps {
                sshagent(credentials: ['pemKey']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no $SSH_CONNECTION 'docker run -d --name $CONTAINER_NAME_BACK --env-file $ENV_DIR -p $PORT_BACK:8000 $DOCKERHUB_REPOSITORY_BACK:$VERSION'"
