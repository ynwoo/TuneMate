pipeline {
    ...
    stages {
        stage('Build Music Backend') {
            steps {
                dir('tunemate-discovery-service') {
                    sh "chmod +x gradlew"
        ...
        stage('Deploy Backend Server') {
            steps {
                sshagent(credentials: ['pemKey']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no $SSH_CONNECTION 'docker run -d --name $CONTAINER_NAME_BACK --env-file $ENV_DIR -p $PORT_BACK:8761 $DOCKERHUB_REPOSITORY_BACK:$VERSION'"
