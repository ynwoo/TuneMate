...
const INDIVIDUAL_PLAYLISTS_URL = `${INDIVIDUAL_URL}/playlists`;
...
const getIndividualPlayListRepresentative = async (): Promise<PlayList> => {
  const response = await api.get<PlayList>(`${INDIVIDUAL_PLAYLISTS_URL}-representative`);
  return response.data;
};

// 개인 플레이리스트 목록조회
const getIndividualPlayLists = async (userId: UserInfo["userId"]): Promise<any> => {
  const response = await spotifyApi.get(`users/${userId}/playlists`);
  return response.data.items;
};

// 개인 대표 플레이리스트 트랙 추가
const createIndividualPlayListTrack = async ({ playlistId, uris, position }: AddTrack) => {
...
// 개인 대표 플레이리스트 트랙 삭제
const deleteIndividualPlayListTrack = async ({ playlistId, uri, positions }: DeleteTrack) => {
...
// 개인 플레이리스트 트랙 순서 변경
const updateIndividualPlayListTrack = async ({ playlistId, changeTrackIndex }: ChangeTrack) => {
  await api.put<void>(`${INDIVIDUAL_PLAYLISTS_URL}/${playlistId}/tracks`, changeTrackIndex);
};
...
const addIndividualMusicCount = async () => {
  await api.post<void>(`${INDIVIDUAL_URL}/count`);
};
